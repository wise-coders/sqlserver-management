...
FROM ${isTempTable?'[tempdb].':''}[sys].[columns] [colz]
    INNER JOIN ${isTempTable?'[tempdb].':''}[sys].[objects] [objz] ON [objz].[object_id] = [colz].[object_id]
    INNER JOIN ${isTempTable?'[tempdb].':''}[sys].[sql_modules] AS [MODS] ON [colz].[rule_object_id] = [MODS].[object_id]
