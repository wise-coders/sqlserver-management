...
FROM   [sys].[foreign_keys] AS [conz]
       ...
       INNER JOIN sys.objects o
			ON conz.referenced_object_id = o.object_id
...
--##############################################################################
  ...
  SELECT  @EXTENDEDPROPERTIES =
         ...
         'EXEC sys.sp_addextendedproperty
          @name = N'''  COLLATE SQL_Latin1_General_CP1_CI_AS + [name] + ''', @value = N'''  COLLATE SQL_Latin1_General_CP1_CI_AS + REPLACE(CONVERT(NVARCHAR(MAX),[value]),'''','''''') + ''',
 ...
 --SELECT objtype, objname, name, value
  ...
  @EXTENDEDPROPERTIES =
         ...
         + REPLACE(CONVERT(NVARCHAR(MAX),[lep].[value]),'''','''''') + ''',
