...
SELECT
    ...
    END
    AS [Command]
FROM ${isTempTable?'[tempdb].':''}[sys].[foreign_keys] AS [conz]
INNER JOIN ${isTempTable?'[tempdb].':''}[sys].[foreign_key_columns] AS [colz] ON [conz].[object_id] = [colz].[constraint_object_id]
INNER JOIN (--gets my child tables column names
    SELECT
       ...
       [ChildColumns] = STUFF((SELECT
         ',' + QUOTENAME([REFZ].[name])
       FROM  ${isTempTable?'[tempdb].':''}[sys].[foreign_key_columns] AS [fkcolz]
       INNER JOIN ${isTempTable?'[tempdb].':''}[sys].[columns] AS [REFZ] ON [fkcolz].[parent_object_id] = [REFZ].[object_id] AND [fkcolz].[parent_column_id] = [REFZ].[column_id]
...
SELECT
    ...
    [ParentColumns] = STUFF((SELECT
          ',' + [REFZ].[name]
        FROM ${isTempTable?'[tempdb].':''}[sys].[foreign_key_columns] AS [fkcolz]
               INNER JOIN ${isTempTable?'[tempdb].':''}[sys].[columns] AS [REFZ]
        ...
        FOR XML PATH(''), TYPE).[value]('.','varchar(max)'),1,1,'')
    FROM  ${isTempTable?'[tempdb].':''}[sys].[foreign_keys] AS [conz]
      INNER JOIN ${isTempTable?'[tempdb].':''}[sys].[foreign_key_columns] AS [colz]
