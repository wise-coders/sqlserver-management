...
-- Obtained from http://www.stormrage.com/SQLStuff/sp_GetDDL_Latest.txt
--#################################################################################################
...
--V319   Adding logic for Temporal Tables, to grab their auto history tables
...
' 
            ...
            BEGIN
               SELECT @FINALSQL = 
                'IF EXISTS(SELECT * FROM sys.synonyms WHERE name = ''' 
                                ...
                                + '  DROP SYNONYM ' + QUOTENAME([name]) + '' + @vbCrLf
                                +'GO' + @vbCrLf
      ...
      ELSE
      --##############################################################################
      -- Check If this is actually a schema instead of an object
      --##############################################################################
      IF EXISTS(SELECT * FROM [sys].[schemas] scmz WHERE [scmz].[name] = @TBLNAME AND @SCHEMANAME = 'dbo' AND [scmz].[schema_id] > 4 AND [scmz].[schema_id] < 16384)
      BEGIN
        SELECT @FINALSQL = 
            ...
            +  '    EXEC (@cmd)' + @vbCrLf
            +  '  END ;' + @vbCrLf
          from [sys].[schemas] [scmz]
          WHERE [scmz].[name] = @TBLNAME
          AND schema_id > 4 
          AND schema_id < 16384;
               SELECT @input AS [Item];
                 RETURN;
